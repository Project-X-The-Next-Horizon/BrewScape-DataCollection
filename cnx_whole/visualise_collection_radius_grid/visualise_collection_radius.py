...
2) Load/calculate Chiang Mai province border GeoJSON (with local cache).
3) Draw circles, center markers, and province border overlay in Folium.
"""
...
def _feature_collection_from_relation_row(row: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(geometry, dict):
        print("Error: Chiang Mai province relation is missing geometry.", file=sys.stderr)
        raise SystemExit(1)
    if geometry.get("type") not in {"Polygon", "MultiPolygon"}:
        print(
            "Error: Chiang Mai province relation has unsupported geometry type "
...
def _fetch_chiang_mai_province_geojson() -> dict[str, Any]:
    ...
    try:
        ...
        print(f"Error: unable to download Chiang Mai province border: {exc}", file=sys.stderr)
        raise SystemExit(1)
    ...
    if not isinstance(data, list) or len(data) != 1:
        print("Error: Chiang Mai province border service returned unexpected results.", file=sys.stderr)
        raise SystemExit(1)

    row = data[0]
    if not isinstance(row, dict) or row.get("osm_type") != "relation":
        print("Error: Chiang Mai province lookup did not return a relation.", file=sys.stderr)
        raise SystemExit(1)

    osm_id = row.get("osm_id")
    if f"R{osm_id}" != CHIANG_MAI_PROVINCE_RELATION_ID:
        print(
            "Error: Chiang Mai province lookup returned the wrong relation "
...
def _build_map(points: list[dict[str, Any]], chiang_mai_border: dict[str, Any]) -> folium.Map:
    ...
    folium.GeoJson(
        chiang_mai_border,
        name=f"{CHIANG_MAI_BORDER_NAME} Border Halo",
    ...
    folium.GeoJson(
        chiang_mai_border,
        name=f"{CHIANG_MAI_BORDER_NAME} Border",
        style_function=lambda _feature: {
        ...
        tooltip=f"{CHIANG_MAI_BORDER_NAME} Border",
    ).add_to(point_map)
